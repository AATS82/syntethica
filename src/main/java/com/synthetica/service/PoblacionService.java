...
public class PoblacionService {
    ...
    private String samplearEducacion(Random rng, int edad, String nse) {
        ...
        if ("Alto".equals(nse)) {
            p[0] *= 0.10; p[1] *= 0.25; p[3] *= 2.5; p[4] *= 4.0;
        } else if ("Medio-alto".equals(nse)) {
            p[0] *= 0.20; p[1] *= 0.50; p[3] *= 2.0; p[4] *= 2.0;
        }
    ...
    private String samplearPrevision(Random rng, String nse) {
        double[] p;
        switch (nse) {
            case "Bajo":      p = new double[]{0.950, 0.020, 0.025, 0.005}; break;
            case "Medio-bajo":p = new double[]{0.900, 0.060, 0.030, 0.010}; break;
            case "Medio":     p = new double[]{0.820, 0.140, 0.030, 0.010}; break;
            case "Medio-alto":p = new double[]{0.600, 0.360, 0.025, 0.015}; break;
            default:          p = new double[]{0.450, 0.500, 0.025, 0.025}; // Alto
        }
    ...
    private String samplearTransporte(Random rng, String nse, String zona) {
        ...
        switch (nse) {
            case "Alto":       { double[] p = {0.70, 0.05, 0.15, 0.07, 0.03}; return samplearConPesos(TRANSPORTE, p, rng); }
            case "Medio-alto": { double[] p = {0.50, 0.15, 0.20, 0.10, 0.05}; return samplearConPesos(TRANSPORTE, p, rng); }
            case "Bajo":       { double[] p = {0.08, 0.58, 0.08, 0.21, 0.05}; return samplearConPesos(TRANSPORTE, p, rng); }
            case "Medio-bajo": { double[] p = {0.15, 0.50, 0.10, 0.20, 0.05}; return samplearConPesos(TRANSPORTE, p, rng); }
            default:           { double[] p = {0.25, 0.35, 0.18, 0.15, 0.07}; return samplearConPesos(TRANSPORTE, p, rng); }
        }
    }

    private String samplearBanco(Random rng, String nse) {
        switch (nse) {
            case "Bajo":       { double[] p = {0.50, 0.05, 0.25, 0.10, 0.10}; return samplearConPesos(BANCO, p, rng); }
            case "Medio-bajo": { double[] p = {0.45, 0.10, 0.20, 0.15, 0.10}; return samplearConPesos(BANCO, p, rng); }
            case "Medio":      { double[] p = {0.35, 0.30, 0.15, 0.15, 0.05}; return samplearConPesos(BANCO, p, rng); }
            case "Medio-alto": { double[] p = {0.20, 0.55, 0.05, 0.15, 0.05}; return samplearConPesos(BANCO, p, rng); }
            default:           { double[] p = {0.10, 0.75, 0.02, 0.10, 0.03}; return samplearConPesos(BANCO, p, rng); } // Alto
        }
    }
    ...
    private int samplearEdad(Random rng, Integer edadMin, Integer edadMax) {
        ...
        if (minEd > maxEd) { minEd = 18; maxEd = 90; }
    ...
    public static class PerfilSintetico {
        ...
        /**
         * Genera el texto que va como system prompt a Claude.
         * Reemplaza a Persona.toPromptContext() con atributos psicográficos adicionales.
         */
        ...
        /**
         * Resumen breve para guardar en Respuesta.perfilResumen.
         * Ej: "María González Muñoz, 34 años, Femenino, Santiago, Universitaria, Medio-alto"
         */
        ...
        public String getPrevisionSalud()      { return previsionSalud; }
        public String getReligion()            { return religion; }
        public String getPersonalidad()        { return personalidad; }
        public String getSensibilidadPrecio()  { return sensibilidadPrecio; }
        public String getAdopcionTecnologica() { return adopcionTecnologica; }
        public String getRedesSociales()       { return redesSociales; }
        public String getTransporte()          { return transporte; }
        public String getPosturaPolitica()     { return posturaPolitica; }
        public String getLealtadMarca()        { return lealtadMarca; }
        public String getAlimentacion()        { return alimentacion; }
        public String getBanco()               { return banco; }
        public String getEndeudamiento()       { return endeudamiento; }
    }
}
